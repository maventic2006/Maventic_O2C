...
import { d_o2c_emp_salary_hdr_log as d_o2c_emp_salary_hdr_log_gen } from "o2c_v2/entity_gen/d_o2c_emp_salary_hdr_log"
import { employeeHierarchy } from "o2c_v2/util/employeeHierarchy";
export class d_o2c_emp_salary_hdr_log extends d_o2c_emp_salary_hdr_log_gen {
    ...
    for (let user_id of Object.keys(userAppRoleMap)) {
      ...
      if (userAppRoleMap[user_id] == "MANAGER" || userAppRoleMap[user_id] == "TEAM_HEAD" || userAppRoleMap[user_id] == "TOP_MANAGEMENT") {
        for (let a of allEmployeeUnderHierarchy) {
          accessTokenMap[user_id].access_tokens.push(await accessTokenHelper.getAccessToken("emp_salary_log", { employee_id: a }))
        }
      }
      accessTokenMap[user_id].access_tokens.push(await accessTokenHelper.getAccessToken("emp_salary_log", { employee_id: user_id }))
    }
    return accessTokenMap;
  }
