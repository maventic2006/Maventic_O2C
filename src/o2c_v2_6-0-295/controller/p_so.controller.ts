...
import { d_o2c_so_type_master } from "o2c_v2/entity_gen/d_o2c_so_type_master";
import { perdiem_updater } from "o2c_v2/util/perdiem_updater";

declare const KloUI5: any;
@KloUI5("o2c_base.controller.p_so")
let projectData;
export default class p_so extends KloController {
    public company_code;
    ...
    public async onPageEnter(oEvent) {
        // --- To Load XML ---
        let view = sap.ui.getCore().byId("ObjPageID");

        if (!view) {
            const sViewName = this.getFlavor() + "_" + this.getFlavorVersion() + "_" + this.transaction.$SYSTEM.landscape + ".view.p_so";
            view = await sap.ui.core.Fragment.load({ name: sViewName, controller: this, }) as sap.ui.core.Control;
        }

        this.getActiveControlById(null, "pageLayout01", "p_so").addContent(view);
        ...
        this.permissionAvlbl = await this.transaction.hasPermission("can_edit_approve_so");
        ...
        //Order Type Data from master table.
        let orderTypeData = <KloEntitySet<d_o2c_so_type_master>>await this.transaction.getExecutedQuery("d_o2c_so_type_master", { loadAll: true });
        const OrderTypeVisiblityTemplate = {
            PS: false,
            NBS: roleId === "TEAM_HEAD",
            ISP: roleId === "TEAM_HEAD",
            PSL: roleId === "TEAM_HEAD",
            ETR: roleId === "TEAM_HEAD",
            HR: roleId === "FINANCE" || this.secondRole?.toUpperCase() === "FINANCE",
            Admin: roleId === "FINANCE" || this.secondRole?.toUpperCase() === "FINANCE",
            IT: roleId === "FINANCE" || this.secondRole?.toUpperCase() === "FINANCE",
        }

        const orderTypesList = [];
        orderTypeData.forEach(oOrdType => {
            orderTypesList.push({
                key: oOrdType.type_id,
                text: oOrdType.type_description,
                isEnabled: OrderTypeVisiblityTemplate[oOrdType.type_id] == undefined ? true : OrderTypeVisiblityTemplate[oOrdType.type_id]
            })
        })

        const financial_impact = orderTypeData.find(item => item.type_id === "SO");
        sap.ui.getCore().getModel("mPageData").setProperty("/orderTypeFinancialImpact", financial_impact.financial_impact);
    ...
    public async onOrderTypeSelectionChange() {
        ...
        const oModel = sap.ui.getCore().getModel("mPageData");
        const order_type_master = <KloEntitySet<d_o2c_so_type_master>>await this.transaction.getExecutedQuery("d_o2c_so_type_master", { loadAll: true, type_id: orderTypeSelected });
        oModel.setProperty("/orderTypeFinancialImpact", order_type_master[0].financial_impact);
    ...
    public async onBusinessAreaChange(oEvent: Event = null) {
        ...
        aTravelData.forEach((oTravelData) => {
            travelCityReimburseMapping.push({
                ...
                bill_to_customer: oTravelData.bill_to_customer,
    ...
    public fnEstimateProjDate(oEvent: Event, inputIdentity: string) {
        ...
        switch (inputIdentity) {
            ...
            case "END_DATE":
                ...
                fetchInputDatesAndWeeks();
                // Validate from PO endDate;
                const poEndDate = this.fnFindEndDateFromPo();
                const orderTypeSelected = sap.ui.getCore().getModel("mPageData").getProperty("/orderTypeSelected");
                if (orderTypeSelected === 'SO' || orderTypeSelected === 'PS') {
                    if (!poEndDate) {
                        sap.ui.getCore().getModel("mPageData").setProperty("/project_end_date", "");
                        this.fnResetSetValueStateErrorNoActPath("/project_end_date", true, "No POs Available.");
                        break;
                    }
                    // else if (dateEnd > poEndDate) {
                    //     sap.ui.getCore().getModel("mPageData").setProperty("/project_end_date", "");
                    //     this.fnResetSetValueStateErrorNoActPath("/project_end_date", true, "End Date Should Smaller than PO End Date.");
                    //     break;
                    // }
                }
    ...
    public onAddOSTravelButtonPress() {
        ...
        tableTravel.push({
            ...
            standardReimburse: "",
            applicableReimburse: "",
            isDirtyPayToEmployeePerDiem: false
        });

        oPageDataModel.setProperty("/tableTravel", tableTravel);
    }

    public onChangeApplicableReimburse(oEvent: Event) {
        ...
        const oPageDataModel = sap.ui.getCore().getModel("mPageData");
        oPageDataModel.setProperty(sRowPath + "/isDirtyPayToEmployeePerDiem", true);
    }
    /**
    ...
    public onTravelTypeDDChange(oEvent: Event) {
        ...
        oPageDataModel.setProperty(sRowPath + "/applicableReimburse", "");
        oPageDataModel.setProperty(sRowPath + "/isDirtyPayToEmployeePerDiem", false);
    ...
    public onToCityDDChange(oEvent: Event) {
        ...
        const applicableReimburse = this.getApplicableReimburseVal(travelTypeSelected, toCitySelected);
        ...
        oPageDataModel.setProperty(sRowPath + "/applicableReimburse", applicableReimburse);
        oPageDataModel.setProperty(sRowPath + "/isDirtyPayToEmployeePerDiem", false);
    ...
    public getStandardReimburseVal(travelType: string, travelCity: string) {
        ...
        if (aTavelCityDropDown) {
            return aTavelCityDropDown.bill_to_customer;
        } else {
            return "N/A";
        }
    }

    public getApplicableReimburseVal(travelType: string, travelCity: string) {
        const aTravelCityReimburseMapping = sap.ui.getCore().getModel("mPageData").getProperty("/travelCityReimburseMapping");
        const aTavelCityDropDown = aTravelCityReimburseMapping.find(oMapping => oMapping.travel_type === travelType && oMapping.travelling_location === travelCity);
    ...
    public async onAttachCopyUpload(oEvent: Event) {
        this.fnInputBoxResetValueState(oEvent);
        const path = oEvent.getSource().getParent().getParent().getBindingContextPath();
        const oPageDataModel = sap.ui.getCore().getModel("mPageData");
        let attach_data = oPageDataModel.getProperty(path);
        oPageDataModel.setProperty(path + "/AttachedCopy", oEvent.mParameters.files[0]);
        await this.attachmentCreation(attach_data, true);
        // if (rowID) {
        //     let attachment = <KloEntitySet<d_o2c_so_attachment>>await this.transaction.getExecutedQuery("d_o2c_so_attachment", { loadAll: true, attachment_id: rowID });
        //     await attachment[0].attach_copy.setAttachmentP(oEvent.mParameters.files[0], oEvent.mParameters.files[0].name);
        // }
    }

    public async onProposalCopyUpload(oEvent: Event) {
        this.fnInputBoxResetValueState(oEvent);
        const path = oEvent.getSource().getParent().getParent().getBindingContextPath();
        const oPageDataModel = sap.ui.getCore().getModel("mPageData");
        let attach_data = oPageDataModel.getProperty(path);
        oPageDataModel.setProperty(path + "/ProposalCopy", oEvent.mParameters.files[0]);
        await this.attachmentCreation(attach_data, true);
        // let rowID = oPageDataModel.getProperty(path).rowID;
        // if (rowID) {
        //     let attachment = <KloEntitySet<d_o2c_so_attachment>>await this.transaction.getExecutedQuery("d_o2c_so_attachment", { loadAll: true, attachment_id: rowID });
        //     await attachment[0].personal_copy.setAttachmentP(oEvent.mParameters.files[0], oEvent.mParameters.files[0].name);
        // }
    }
    ...
    public askUserConfirmation() {
        return new Promise(function (resolve) {
            sap.m.MessageBox.confirm("Manual Data Entry ?", {
                title: "Decide",
                actions: ["Yes", "No"],
    ...
    public async fnDecideMilestoneTableManualCreation(oReference: { oPoHeaderTblLineItemRow: any }) {
        const oPoHeaderTblLineItemRow = oReference.oPoHeaderTblLineItemRow;
        let isManuallyAddRow;

        // Ask for popup
        if (oPoHeaderTblLineItemRow.isLastLineTableManualEntry === undefined) {
            isManuallyAddRow = await this.askUserConfirmation();
            oPoHeaderTblLineItemRow.isLastLineTableManualEntry = isManuallyAddRow;
        } else {
            isManuallyAddRow = oPoHeaderTblLineItemRow.isLastLineTableManualEntry;
        }

        return isManuallyAddRow;
    }

    public async fnAddLastLineItemTable(oEvent: Event): void {
        const sPath: string = oEvent.getSource().getParent().getBindingContextPath();
        const oPageDataModel = sap.ui.getCore().getModel("mPageData");
        ...
        switch (sTableIdentifier) {
            ...
            case "TABLE_SCHEDULE_BASED":
                let isManuallyAddRow: any = await this.fnDecideMilestoneTableManualCreation({ oPoHeaderTblLineItemRow });

                if (isManuallyAddRow) {
                    oPoHeaderTblLineItemRow.aLastLineItemTableIdentity = sTableIdentifier;
                    oPoHeaderTblLineItemRow.aLastLineItemTable.push(
                        {
                            isManuallyAddedItem: true,
                            isPoRowEditable: true,
                            rowID: "",
                            description: "",
                            startDate: "",
    ...
    public async onSoLineItemSelectionChange3_BySource(oSource: any) {
        ...
        const sRowPath = oSource.getParent().getBindingContextPath();
        const oPageDataModel = sap.ui.getCore().getModel("mPageData");
        const oPoHeaderTblLineItemRow = oPageDataModel.getProperty(sRowPath);

        let isManuallyAddRow: any = await this.fnDecideMilestoneTableManualCreation({ oPoHeaderTblLineItemRow });
        if (isManuallyAddRow) return;
    ...
    public onMileStoneTablePercentChange(oEvent: Event, isAmountInput = false) {
        ...
        if (TotalPercent >= 100 && Math.abs(TotalPercent - 100) >= 0.02) { // +0.01xxxx
    ...
    public fnFindAndAssignLastDate() {
        ...
        const lastDate = this.fnFindEndDateFromPo();
        oModel.setProperty("/project_end_date", this.fnConvertJsToUi5Date(lastDate));
        this.fnResetSetValueStateErrorNoActPath("/project_end_date", false);
    ...
    public async onEndDateChangeTableLItem_2(oEvent: Event) {
        ...
        // If Table is Already Volume Based  && Other Than AMC
        if (aLastLineItemTableIdentity === "TABLE_VOLUME_BASED" && ddSelectedItemCategory !== "AMC") {
            let isManuallyAddRow: any = await this.fnDecideMilestoneTableManualCreation({ oPoHeaderTblLineItemRow });
            if (isManuallyAddRow) return;
    ...
    public fnValidateDataBeforeSubmit() {
        ...
        if (orderTypeSelected === "SO" || orderTypeSelected === "PS") {
            ...
            for (let i = 0; i < poHeaderTable.length; i++) {
                ...
                const aErrorsInPO_L1 = [
                    this.isInvalidInputBoxEntry(sPathL_1 + "/AttachCopyName", VAL_TYPES.ANY, DocType !== "AM" && orderTypeSelected !== "PS"),
        ...
        if (orderTypeSelected === "SO" || orderTypeSelected === "PS" || orderTypeSelected === "ISP") {
            // * Onsite Rules
    ...
    public validateProfitCentrePercentage(sCurrentRowPath: string) {
        ...
        if (totalPercent >= 100 && Math.abs(totalPercent - 100) >= 0.02) { // +0.01xxxxx
    ...
    public async fnFetchAndAutoFillSOData(guid) {
        ...
        const so_info = <KloEntitySet<d_o2c_so_hdr>>await this.transaction.getExecutedQuery("d_o2c_so_hdr", { loadAll: true, so_guid: guid });
        const so_type_master = <KloEntitySet<d_o2c_so_type_master>>await this.transaction.getExecutedQuery("d_o2c_so_type_master", { loadAll: true, type_id: so_info[0].type });
        const customerSalesResponsible = <KloEntitySet<d_o2c_customers>>await this.transaction.getExecutedQuery("d_o2c_customers", { loadAll: true, customer_id: so_info[0].bill_to_customer });
        this.header_detail = so_info[0];
        oModel.setProperty("/orderTypeFinancialImpact", so_type_master[0].financial_impact);
        ...
        for (let i = 0; i < this.poTableLineItem1.length; i++) {
            ...
            if (poHeaderRow.approval_status === "Save As Draft") {
                ...
                if (approvalRole.length) {
                    // Display Row
                } else if (login_id === poHeaderRow.s_created_by || loginUserRole === "MANAGER" || this.permissionAvlbl || login_id.toLowerCase() === (so_info[0].sales_responsible).toLowerCase()) {
                    // Display Row
                ...
                if (loginUserRole === "MANAGER" || loginUserRole === "SALES" || this.permissionAvlbl) {
        ...
        for (let i = 0; i < this.profit_center.length; i++) {
            ...
            if (loginUserRole === 'MANAGER' || loginUserRole === 'TEAM_HEAD' || this.permissionAvlbl) {
    ...
    public async fnSubmitData() {
        ...
        //PS TO SO
            ...
            if (
                ...
                this.header_detail.pstoso == 1 || this.ppoTOpo == 1
            ) {
                this.header_detail.s_status = "New"; //Commented out because while converting ppo to po it was changing all po status to pending. 
    ...
    public async dataFillInTable() {
        ...
        for (let i = 0; i < data.poHeaderTable.length; i++) {
            ...
            this.attachmentCreation(element, false);
            ...
            // }
            // else if (POTableData[0].approval_status != "New") {
            //     POTableData[0].approval_status = element.approval_status;
            // }
            ...
            for (let j = 0; j < po_line_item.length; j++) {
                ...
                for (let i = 0; i < item_element.ddSelectedPOSubscription.length; i++) {
                    ...
                    if (!apiTypeData.length) {
                        ...
                        item_element.aSavedPoSubscriptionIdMap.push({ subscriptionKey: item_element.ddSelectedPOSubscription[i], subscriptionId: apiTypeData[0].table_id });
                    }
                ...
                if (sTableIdentifier != "") {
                    ...
                    for (let k = 0; k < aLastLineItemTable.length; k++) {
                        ...
                        if (o2cTableMapping == "d_o2c_so_milestone") {
                            ...
                            billingTableData[0].invoice_description = billing_element.invoice_description;
                        }
    ...
    public async fnSaveAsDraftData() {
        ...
        //PS TO SO
            ...
            if (
                data.orderTypeSelected == "SO" &&
                order_type == "PS" &&
                this.header_detail.s_status == "Approved" || this.ppoTOpo == 1
            ) {
                //this.header_detail.s_status = "New";
                this.header_detail.pstoso = 1;
            }
    ...
    public async onSave() {
        ...
        if (this.saveFlag == "true") {
            ...
            // await this.tm.commitP("Save successfull", "Save Failed", true, true);
            await this.retrySave("Save successfull", "Save Failed");
            ...
            if (this.header_detail.s_status == "New" || this.header_detail.s_status == "Back To Edit" || this.header_detail.s_status == "Call Back") {
                for (let i = 0; i < newPO.length; i++) {
                    if ((this.header_detail.s_status == "New" && this.header_detail.pstoso === 0) || this.header_detail.s_status == "Call Back")
            ...
            if (this.header_detail.s_created_by == userloginid) {
                ...
                if (login) {
                    ...
                    this.fnFetchAndRefillComments(sap.ui.getCore().getModel("mPageData"), this.header_detail, true);
                } else {
                    await this.navTo({ S: "p_sales_order_list" });
                }
            }
            ...
            // if (this.header_detail.s_status == "Approved")
            //     this.header_detail.cr_status = "Save As Draft";
            // await this.tm.commitP("Save As Draft", "Save As Draft Failed", true, false);
            await this.retrySave("Save As Draft", "Save As Draft Failed");
            sap.m.MessageBox.confirm(
        ...
        if (this.header_detail.so != undefined)
            sap.ui.getCore().getModel("mPageData").setProperty("/soNumber", this.header_detail.so);
    }
                ...
                "This will be returned back to the creator for editing. Are you sure ?",
                {
                    ...
                    onClose: async (oAction) => {
                        if (oAction == "YES") {
                            ...
                            this.tm.commitP("Back To Edit Successfull","Back To Edit Failed",true,true);
    ...
    public async onPressApproveBtn() {
        ...
        if (currStatus === "Call Back") {
            ...
            await this.retrySave("Approved Successfull", "Approved Failed");
    ...
    public async onSubmitMail() {
        ...
        for (let i = 0; i < po_header.length; i++) {
            ...
            if (po_header[i].approval_status != "Approved") {
                poNumber += po_header[i].PONumber + ",";
            }
        ...
        for (let i = 0; i < profitCenterTable.length; i++) {
            ...
            await this.tm.getTN("so_notification").setProperty("poNumber", poNumber);
            ...
            await this.tm.getTN("so_notification").setProperty("approver", profitCenterTable[i].manager.toLowerCase());
            await this.tm.getTN("so_notification").executeP();
        }

        sap.m.MessageToast.show(
            "Reminder Mail for Sales Order is sent successfully!"
        );
    }
    ...
    public async alreadyCreatedProject() {
        ...
        for (let i = 0; i < data.profitCenterTable.length; i++) {
            ...
            if (projectPresent.length > 0) {
                ...
                projectPresent[0].profit_center = data.profitCenterTable[i].profitCentre;
            }
    ...
    public onDocTypeChange(oEvent) {
        ...
        //get curr selected item key
        const sRowPath = oSource.getParent().getBindingContextPath();
        const DocType = oPageDataModel.getProperty(sRowPath + "/DocType");
        let so_attachment = this.poTableLineItem1.find(item => item.attachment_type == "AM")
        if (DocType == "POA" && so_attachment && this.header_detail.s_status == "Approved") {
            oPageDataModel.setProperty(sRowPath + "/approval_status", "Pending");
            this.ppoTOpo = 1;
        } else {
            this.ppoTOpo = 0;
        }
    ...
    // Creating and setting the attachment at the time of upload in db instead on save event.
    public async attachmentCreation(element, file_uploaded: boolean) {
        let POTableData = this.poTableLineItem1.filter((item) => item.attachment_id == element.rowID);
        if (!POTableData.length) {
            const attachment = await this.transaction.getQueryP("d_o2c_so_attachment");
            attachment.setLoadAll(true);
        ...
        if (!file_uploaded) {
            POTableData[0].so_guid = this.header_detail.so_guid;
        }
        if (POTableData[0].approval_status == "Save As Draft" || POTableData[0].approval_status == "Return Back") {
            POTableData[0].approval_status = "New";
        }
        ...
        POTableData[0].attachment_type = element.DocType;
        // File - Start
        if (file_uploaded && element.AttachedCopy != "")
            await POTableData[0].attach_copy.setAttachmentP(element.AttachedCopy, element.AttachedCopy.name);
        if (file_uploaded && element.ProposalCopy != "")
            await POTableData[0].personal_copy.setAttachmentP(element.ProposalCopy, element.ProposalCopy.name);
        // File - End
        POTableData[0].po_no = element.PONumber;
        POTableData[0].cr_no = element.CRNumber;
