...
@KloUI5("o2c_v2.controller.p_pms_self_rev_hdr")
export default class p_pms_self_rev_hdr extends KloController {
	public async onPageEnter(oEvent) {
		...
		if (oEvent?.navToParams?.AD && key !== "upcomingReview") {
			...
			let listData = await this.tm.getTN("pms_goal_review_hdr_search").executeP();
			//finding the earlier deadline
			// let earlierDate = new Date(new Date().setFullYear(new Date().getFullYear() + 1));
			// listData.forEach((item) => {
			// 	if (item.review_end_date < earlierDate) {
			// 		earlierDate = item.review_end_date;
			// 	}
			// });
			if (listData.length > 0) {
				let mBox = await this.getMessageBox();
				mBox.alert("Some of your KPI reviews have upcoming end dates. Please complete them on time.");
			}
			// mBox.alert(`Certain KPI reviews are nearing their deadlines. The earliest closure is on ${new Intl.DateTimeFormat("en-IN", { dateStyle: "medium" }).format(earlierDate)}`);
		}
	...
	public checkEmployeeAchievement() {
		...
		return allKpiData.filter((e) => !e.actual_value).length > 0 ? true : false;
	}
	public async onSubmit() {
		let messageBox = await this.getMessageBox();
		let bDialog = new sap.m.BusyDialog({ text: "Submitting achieved value, please wait..." });
		...
		if (isEmpAchiementSub) return sap.m["MessageToast"].show("Cannot submit — achieved values is missing for KPIs.");
		await messageBox.confirm("Are you sure you want to submit?", {
			...
			onClose: async (oAction) => {
				if (oAction == "YES") {
					try {
						bDialog.open();
						this.tm.getTN("pms_goal_review_hdr_detail").setProperty("is_emp_submitted_review", 1);
