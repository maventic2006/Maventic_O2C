...
@KloUI5("o2c_v2.controller.p_pa_grading")
export default class p_pa_grading_copy extends KloController {
    ...
    public async onPageEnter() {
        ...
        if (general_config[0].high_value == 1) {
            const waitForLogin = new Promise<void>((resolve, reject) => {
                ...
                this._rejectLoginPromise = reject;
            });
            ...
            try {
                ...
                console.warn("Login was cancelled or failed:", error);
                // Optionally handle failure (e.g., show error message or exit flow)
            }
        }
    }
    ...
    public async onCancel() {
        if (this._rejectLoginPromise) {
            this._rejectLoginPromise("Login cancelled by user");
            this._rejectLoginPromise = null;
            this._resolveLoginPromise = null;
        }
    ...
    public async manteeData(fiscalSDate, fiscalEDate) {
        ...
        await this.tm.getTN("pa_search_data").setProperty('line_manager', searchData.loginUser);
        //await this.tm.getTN("pa_search_data").setProperty('business_area', searchData.businessArea);
        await this.tm.getTN("pa_search_data").setProperty('profit_center', searchData.profitCenter);
        await this.tm.getTN("pa_search_data").setProperty('fyearstartdate', fiscalSDate);
        await this.tm.getTN("pa_search_data").setProperty('fyearenddate', fiscalEDate);
        await this.tm.getTN("pa_search_data").executeP();
        await this.lineManagerData();
        let list = await this.tm.getTN("pa_employee_list").getData();

        this.paCycleTableData = await this.fnFetchPACycleData(searchData.fyear)

        this.empGrading = this.paCycleTableData[0]?.r_pa_emp_planning_hdr;
        ...
        if (list) {
            for (let emp of list) {
                ...
                let empPAPlanning;
                if (this.empGrading) {
                    empPAPlanning = this.empGrading.find(item => item.employee_id === emp.employee_id);
                }
                if (empPAPlanning) {
                    emp['prev_grading'] = empPAPlanning.pa_category_id;
                    emp['hdr_pa_req_id'] = empPAPlanning.hdr_pa_req_id;
                    emp['pa_exp'] = empPAPlanning.pa_exp;
                } else {

                    const PAFromDate = await this.paExperience(emp.employee_id);
                    // emp['pa_exp'] = PAFromDate
                    // 	? await paFinancialYear.experienceInYear(PAFromDate, emp.confirmation_date)
                    // 	: 0;
                    //code
                    let maventicPAExperience;
                    if (PAFromDate) {
                        maventicPAExperience = await paFinancialYear.experienceInYear(PAFromDate, emp.confirmation_date);
                    }
    ...
    public async fnFetchPACycleData(fyear) {
        ...
        await this.tm.getTN("emp_pa_search_data").setProperty('totp', this.code);
        try {
    ...
    public async onChangeGrading(oEvent) {
        ...
        let index = parseInt(path.replace("/pa_employee_list/", ""));
        let list = await this.tm.getTN("pa_employee_list").getData()[index];
    ...
    public async updateGradingData(list) {
        ...
        if (list.hdr_pa_req_id) {
            ...
            if (employeeData && employeeData.length > 0) {
                employeeData[0].pa_category_id = list.prev_grading;
                employeeData[0].pa_exp = list.pa_exp;
            }
        }
        else {
            ...
            let prevPACycleDataBasedOnYear;
            try {
                prevPACycleDataBasedOnYear = <KloEntitySet<d_o2c_emp_pa_cycle>>await this.transaction.getExecutedQuery('q_o2c_employee_pa_cycle', { loadAll: true, 'company_code': searchData.companyCode, 'business_area': searchData.businessArea, 'profit_center': searchData.profitCenter, 'fyear': previousStartYear + "-" + previousEndYear, 'totp': this.code });
            }
            catch (e) {
                busyDialog.close();
                if (e.message.includes("Unauthorized")) {
            ...
            let prevEmployeePAData = prevPACycleDataBasedOnYear[0]?.r_pa_emp_planning_hdr.filter((item) => item.employee_id == list.employee_id)
            //PA Experience
            ...
            //let employeePAData = pACycleData?.[0].r_pa_emp_planning_hdr.filter((item) => item.employee_id == list.employee_id)

            //PA Month
            //let empSalaryData = <KloEntitySet<d_o2c_employee_salary_hdr>>await this.transaction.getExecutedQuery('d_o2c_employee_salary_hdr', { loadAll: true,'totp': this.code, employee_id: list.employee_id, expandAll: 'r_salary_hdr_items' });
            let empSalaryData;
            try {
                empSalaryData = <KloEntitySet<q_o2c_employee_salary_hdr>>await this.transaction.getExecutedQuery('q_o2c_employee_salary_hdr', { loadAll: true, 'totp': this.code, emp_id: list.employee_id, expandAll: 'r_salary_hdr_items' });
            }
            catch (e) {
                if (e.message.includes("Unauthorized")) {
                    busyDialog.close();
            ...
            if (prevEmployeePAData) {
                ...
                fromDate = new Date(empSalaryData?.[0]?.to_date);
                new Date(fromDate.setDate(fromDate.getDate() + 1));
                toDate = new Date(fromDate);
                new Date(toDate.setFullYear(toDate.getFullYear() + 1));
                // prevPADate = new Date(list.confirmation_date);
                const years = searchData.fyear.split('-');
                const baseStartYear = parseInt(years[0]);
                if (list.confirmation_date.getFullYear() == baseStartYear + 1) {
                    prevPADate = "";
                }
                else {
                    prevPADate = empSalaryData?.[0]?.from_date;
                }
            ...
            let empPlanningHdr = <KloEntitySet<d_pa_ind_emp_planning_hdr>>await this.transaction.createEntityP("d_pa_ind_emp_planning_hdr", { pa_cycle_id: this.paCycleTableData[0].pa_cycle_id, employee_id: list.employee_id, approval_cycle: 0, pa_category_id: list.prev_grading, pa_exp: list.pa_exp, from_date: fromDate, to_date: toDate, employee_designation_prev: list.designation, line_manager_id: list.line_manager, pa_month: monthName, prev_pa_date: prevPADate, new_pa_date: newPADate, delta: deltaMonth, total_cost_prev: totalCostPrev, total_cost_new: 0, ctc_prev: ctcPrev, ctc_new: 0, fixed_prev: fixedPrev, fixed_new: 0, s_status: "Pending" });
            ...
            for (let benefit of benefitMap) {
                ...
                await empPlanningHdr.r_pa_emp_ind_hdr_itm.newEntityP(0, {
                    ...
                    start_date_prev: perviousBonusData?.[0]?.start_date,
                    end_date_prev: perviousBonusData?.[0]?.end_date
                });
                //}
    ...
    public async onSubmit() {
        ...
        //else run below code
        let totalExp = [];
        let empData = await this.tm.getTN("pa_employee_list").getData();
        let gradingNullCount = 0;
        for (let i = 0; i < empData.length; i++) {
            //let empTotalExp = await this.tm.getTN("o2c_employee_list").getData()[i].total_experience;
            let empTotalExp = Math.floor(await this.tm.getTN("pa_employee_list").getData()[i].pa_exp);
    ...
    public async categoryPlanningUpdate(allExperience, paCycleData, categoryHdr) {
        ...
        const empList = await this.tm.getTN("pa_employee_list").getData();
            ...
            for (const exp of experience) {
                for (const grade of gradingList) {
                    ...
                    const noOfEmployeeBasedOnExp = empList.filter(
                        (item) => Math.floor(item.pa_exp) == exp && item.prev_grading == grade.key
                    );
    ...
    // }
    public async paExperience(employeeID) {
        //let empSalaryData = <KloEntitySet<d_o2c_employee_salary_hdr>>await this.transaction.getExecutedQuery('d_o2c_employee_salary_hdr', { loadAll: true, employee_id: employeeID,'totp': this.code });
        let empSalaryData
        try {
            empSalaryData = <KloEntitySet<q_o2c_employee_salary_hdr>>await this.transaction.getExecutedQuery('q_o2c_employee_salary_hdr', { loadAll: true, emp_id: employeeID, 'totp': this.code });
        }
        catch (e) {
            if (e.message.includes("Unauthorized")) {
                const waitForLogin = new Promise<void>((resolve) => {
